...
private const val BRIEF_DESCRIPTION = "Usage of arbitrary colors is not allowed"
private const val EXPLANATION = "Use pre-defined colors"

private const val COLORS_FILE = "colors.xml"

@Suppress("UnstableApiUsage")
class ArbitraryColorsDetector : ResourceXmlDetector() {
    ...
    companion object {
        val ISSUE = Issue.create(
            ...
            implementation = Implementation(
                ArbitraryColorsDetector::class.java,
    ...
    override fun visitAttribute(context: XmlContext, attribute: Attr) {
        ...
        if (value.isNullOrBlank() || !value.isColor())
            return
    ...
    override fun visitElement(context: XmlContext, element: Element) {
        if (!context.file.path.contains(COLORS_FILE))
            return
