...
private const val BRIEF_DESCRIPTION = "GlobalScope usage is not allowed"
private const val EXPLANATION = "Using GlobalScope is considered as anti-pattern"
...
class GlobalScopeDetector : Detector(), Detector.UastScanner {
    ...
    private fun createFix(
        ...
        if (node.hasClass(context, ANDROIDX_VIEWMODEL_CLASS)) {
            return createScopeFix(
                context = context,
        ...
        if (node.hasClass(context, ANDROIDX_FRAGMENT_CLASS)) {
            return createScopeFix(
                context = context,
    ...
    private fun createScopeFix(
        ...
        return context.hasDependency(artifact)
            ?.let {
                fix()
                    .replace()
                    .text(GLOBAL_SCOPE)
