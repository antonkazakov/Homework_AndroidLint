...
class RawColorUsageDetector : ResourceXmlDetector() {
    ...
    override fun appliesTo(folderType: ResourceFolderType) =
        folderType in setOf(ResourceFolderType.DRAWABLE, ResourceFolderType.LAYOUT, ResourceFolderType.COLOR, ResourceFolderType.VALUES)

    override fun getApplicableAttributes(): List<String> = XmlScannerConstants.ALL

    override fun visitDocument(context: XmlContext, document: Document) {
        if (context.file.path.endsWith("res/values/colors.xml") && document.documentElement.nodeName == "resources") {
